...
(function () {
  ...
  // Block copying, cutting, context menu, text selection and image dragging
  ['copy', 'cut', 'contextmenu', 'selectstart', 'dragstart'].forEach(evt => {
    document.addEventListener(evt, e => {
      // Allow input typing in PIN boxes
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          entry.target.style.transition = 'opacity 0.8s ease-out, transform 0.8s ease-out, filter 0.4s ease';
